...
class UserController extends Controller
{
    ...
    function userSignup(Request $request){
        ...
        if($user){
            ...
            if (Session::has('quiz-url')){
                ...
                return redirect('/')->with('message',"User Login successfully, Please check email to verify account "); 
            }
        }
    }
    ...
    function userLogin(Request $request){
        ...
        if(!$user || !Hash::check($request->password,$user->password)){
            return redirect('user-login')->with('message-error',"User not valid, Please check mail and password again");
        }
        ...
        if($user){
            ...
            function userLoginQuiz(){
                session::put('quiz-url',url()->previous());
                return view('user-login')->with('message-success',"New password is set, Please login with new password");
            }

            function mcq($id,$name){
                // return $name;
                // return Session::get('firstMCQ')->quiz_id;
        ...
        function verifyUser($email){
           ...
           if($user){
            $user->active=2;
            if($user->save()){
                return redirect('/')->with('message',"User verified successfully");
            }
           }
        }
