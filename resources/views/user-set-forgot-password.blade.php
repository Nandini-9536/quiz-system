    ...
    <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-sm">
        ...
        <form action="/user-set-forgot-password" method="post" class="space-y-4">
            ...
            <div>
                ...
                <input type="hidden" placeholder ="Enter user email" value="{{$email}}"  name="email" class="w-full py-2 px-4 border border-gray-300 rounded-xl focus:outline-none
                ">
                @error('email')
                <div class="text-red-500">{{$message}}</div>
                @enderror
            ...
            <button type="submit" class="w-full bg-blue-500 rounded-xl py-2 px-4 text-white">Update Password</button>
        </form>
    </div>
</div>  
</body>
